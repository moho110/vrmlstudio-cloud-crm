...
@RestController
@RequestMapping("/api")
public class CmfCrmChanceRateController {
    ...
    @GetMapping(value = "/sales/crmchancerate/find/{id}")
    public ResponseEntity<Object> find(@PathVariable("id") Integer id) throws IOException {
        return new ResponseEntity<>(cmfCrmChanceRateService.selectCmfCrmChanceRateById(id), HttpStatus.OK);
    }
     ...
     * uniapp端调用
     *
     * @param cmfCrmChanceRate
     * @return
     */
    @ApiResponse(code = 400, message = "获取帐号操作列表失败")
    @ApiOperation(value = "帐号操作列表", notes = "获取帐号操作列表")
    @GetMapping(value = "/sales/crmchancerate/info/")
    public ResponseEntity<Object> info(CmfCrmChanceRate cmfCrmChanceRate) throws IOException{
        return new ResponseEntity<>(cmfCrmChanceRateService.selectCmfCrmChanceRateList(cmfCrmChanceRate), HttpStatus.OK);
    }
}