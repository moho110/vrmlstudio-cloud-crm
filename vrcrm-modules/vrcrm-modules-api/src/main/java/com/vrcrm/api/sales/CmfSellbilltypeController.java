...
@RestController
@RequestMapping("/api")
public class CmfSellbilltypeController {
    ...
    @GetMapping(value = "/sales/sellbilltype/find/{id}")
    public ResponseEntity<Object> find(@PathVariable("id") Integer id) throws IOException {
        return new ResponseEntity<>(cmfSellbilltypeService.selectCmfSellbilltypeById(id), HttpStatus.OK);
    }
     ...
     * uniapp端调用
     *
     * @param cmfSellbilltype
     * @return
     */
    @ApiResponse(code = 400, message = "获取帐号操作列表失败")
    @ApiOperation(value = "帐号操作列表", notes = "获取帐号操作列表")
    @GetMapping(value = "/sales/sellbilltype/info/")
    public ResponseEntity<Object> info(CmfSellbilltype cmfSellbilltype) throws IOException{
        return new ResponseEntity<>(cmfSellbilltypeService.selectCmfSellbilltypeList(cmfSellbilltype), HttpStatus.OK);
    }
}