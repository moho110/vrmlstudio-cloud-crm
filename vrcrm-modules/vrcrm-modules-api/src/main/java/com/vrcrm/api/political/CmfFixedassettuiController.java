...
@RestController
@RequestMapping("/api")
public class CmfFixedassettuiController {
    ...
    @GetMapping(value = "/political/fixedassettui/find/{id}")
    public ResponseEntity<Object> find(@PathVariable("id") Integer id) throws IOException {
        return new ResponseEntity<>(cmfFixedassettuiService.selectCmfFixedassettuiById(id), HttpStatus.OK);
    }
     ...
     * uniapp端调用
     *
     * @param cmfFixedassettui
     * @return
     */
    @ApiResponse(code = 400, message = "获取帐号操作列表失败")
    @ApiOperation(value = "帐号操作列表", notes = "获取帐号操作列表")
    @GetMapping(value = "/political/fixedassettui/info/")
    public ResponseEntity<Object> info(CmfFixedassettui cmfFixedassettui) throws IOException{
        return new ResponseEntity<>(cmfFixedassettuiService.selectCmfFixedassettuiList(cmfFixedassettui), HttpStatus.OK);
    }
}