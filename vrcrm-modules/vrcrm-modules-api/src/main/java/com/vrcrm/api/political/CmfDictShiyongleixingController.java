...
@RestController
@RequestMapping("/api")
public class CmfDictShiyongleixingController {
    ...
    @GetMapping(value = "/political/shiyongleixing/find/{id}")
    public ResponseEntity<Object> find(@PathVariable("id") Integer id) throws IOException {
        return new ResponseEntity<>(cmfDictShiyongleixingService.selectCmfDictShiyongleixingById(id), HttpStatus.OK);
    }
     ...
     * uniapp端调用
     *
     * @param cmfDictShiyongleixing
     * @return
     */
    @ApiResponse(code = 400, message = "获取帐号操作列表失败")
    @ApiOperation(value = "帐号操作列表", notes = "获取帐号操作列表")
    @GetMapping(value = "/political/shiyongleixing/info/")
    public ResponseEntity<Object> info(CmfDictShiyongleixing cmfDictShiyongleixing) throws IOException{
        return new ResponseEntity<>(cmfDictShiyongleixingService.selectCmfDictShiyongleixingList(cmfDictShiyongleixing), HttpStatus.OK);
    }
}