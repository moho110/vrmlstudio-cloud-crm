...
@RestController
@RequestMapping("/api")
public class CmfShoupiaorecordController {
    ...
    @GetMapping(value = "/sales/shoupiaorecord/find/{id}")
    public ResponseEntity<Object> find(@PathVariable("id") Integer id) throws IOException {
        return new ResponseEntity<>(cmfShoupiaorecordService.selectCmfShoupiaorecordById(id), HttpStatus.OK);
    }
     ...
     * uniapp端调用
     *
     * @param cmfShoupiaorecord
     * @return
     */
    @ApiResponse(code = 400, message = "获取帐号操作列表失败")
    @ApiOperation(value = "帐号操作列表", notes = "获取帐号操作列表")
    @GetMapping(value = "/sales/shoupiaorecord/info/")
    public ResponseEntity<Object> info(CmfShoupiaorecord cmfShoupiaorecord) throws IOException{
        return new ResponseEntity<>(cmfShoupiaorecordService.selectCmfShoupiaorecordList(cmfShoupiaorecord), HttpStatus.OK);
    }
}