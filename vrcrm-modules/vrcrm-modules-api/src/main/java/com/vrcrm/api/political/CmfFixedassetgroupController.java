...
@RestController
@RequestMapping("/api")
public class CmfFixedassetgroupController {
    ...
    @GetMapping(value = "/political/fixedassetgroup/find/{id}")
    public ResponseEntity<Object> find(@PathVariable("id") Integer id) throws IOException {
        return new ResponseEntity<>(cmfFixedassetgroupService.selectCmfFixedassetgroupById(id), HttpStatus.OK);
    }
     ...
     * uniapp端调用
     *
     * @param cmfFixedassetgroup
     * @return
     */
    @ApiResponse(code = 400, message = "获取帐号操作列表失败")
    @ApiOperation(value = "帐号操作列表", notes = "获取帐号操作列表")
    @GetMapping(value = "/political/fixedassetgroup/info/")
    public ResponseEntity<Object> info(CmfFixedassetgroup cmfFixedassetgroup) throws IOException{
        return new ResponseEntity<>(cmfFixedassetgroupService.selectCmfFixedassetgroupList(cmfFixedassetgroup), HttpStatus.OK);
    }
}