...
@RestController
@RequestMapping("/api")
public class CmfOfficeguihuanstateController {
    ...
    @GetMapping(value = "/political/officeguihuanstate/find/{id}")
    public ResponseEntity<Object> find(@PathVariable("id") Integer id) throws IOException {
        return new ResponseEntity<>(cmfOfficeguihuanstateService.selectCmfOfficeguihuanstateById(id), HttpStatus.OK);
    }
     ...
     * uniapp端调用
     *
     * @param cmfOfficeguihuanstate
     * @return
     */
    @ApiResponse(code = 400, message = "获取帐号操作列表失败")
    @ApiOperation(value = "帐号操作列表", notes = "获取帐号操作列表")
    @GetMapping(value = "/political/officeguihuanstate/info/")
    public ResponseEntity<Object> info(CmfOfficeguihuanstate cmfOfficeguihuanstate) throws IOException{
        return new ResponseEntity<>(cmfOfficeguihuanstateService.selectCmfOfficeguihuanstateList(cmfOfficeguihuanstate), HttpStatus.OK);
    }
}