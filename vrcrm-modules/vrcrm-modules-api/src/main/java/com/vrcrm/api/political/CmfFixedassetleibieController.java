...
@RestController
@RequestMapping("/api")
public class CmfFixedassetleibieController {
    ...
    @GetMapping(value = "/political/fixedassetleibie/find/{id}")
    public ResponseEntity<Object> find(@PathVariable("id") Integer id) throws IOException {
        return new ResponseEntity<>(cmfFixedassetleibieService.selectCmfFixedassetleibieById(id), HttpStatus.OK);
    }
     ...
     * uniapp端调用
     *
     * @param cmfFixedassetleibie
     * @return
     */
    @ApiResponse(code = 400, message = "获取帐号操作列表失败")
    @ApiOperation(value = "帐号操作列表", notes = "获取帐号操作列表")
    @GetMapping(value = "/political/fixedassetleibie/info/")
    public ResponseEntity<Object> info(CmfFixedassetleibie cmfFixedassetleibie) throws IOException{
        return new ResponseEntity<>(cmfFixedassetleibieService.selectCmfFixedassetleibieList(cmfFixedassetleibie), HttpStatus.OK);
    }
}