...
@RestController
@RequestMapping("/api")
public class CmfStoreaccesstypeController {
    ...
    @GetMapping(value = "/store/storeaccesstype/find/{id}")
    public ResponseEntity<Object> find(@PathVariable("id") Integer id) throws IOException {
        return new ResponseEntity<>(cmfStoreaccesstypeService.selectCmfStoreaccesstypeById(id), HttpStatus.OK);
    }
     ...
     * uniapp端调用
     *
     * @param cmfStoreaccesstype
     * @return
     */
    @ApiResponse(code = 400, message = "获取帐号操作列表失败")
    @ApiOperation(value = "帐号操作列表", notes = "获取帐号操作列表")
    @GetMapping(value = "/store/storeaccesstype/info/")
    public ResponseEntity<Object> info(CmfStoreaccesstype cmfStoreaccesstype) throws IOException{
        return new ResponseEntity<>(cmfStoreaccesstypeService.selectCmfStoreaccesstypeList(cmfStoreaccesstype), HttpStatus.OK);
    }
}