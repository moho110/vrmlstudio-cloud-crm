...
@RestController
@RequestMapping("/api")
public class CmfOfficeproductbaofeiController {
    ...
    @GetMapping(value = "/political/officeproductbaofei/find/{id}")
    public ResponseEntity<Object> find(@PathVariable("id") Integer id) throws IOException {
        return new ResponseEntity<>(cmfOfficeproductbaofeiService.selectCmfOfficeproductbaofeiById(id), HttpStatus.OK);
    }
     ...
     * uniapp端调用
     *
     * @param cmfOfficeproductbaofei
     * @return
     */
    @ApiResponse(code = 400, message = "获取帐号操作列表失败")
    @ApiOperation(value = "帐号操作列表", notes = "获取帐号操作列表")
    @GetMapping(value = "/political/officeproductbaofei/info/")
    public ResponseEntity<Object> info(CmfOfficeproductbaofei cmfOfficeproductbaofei) throws IOException{
        return new ResponseEntity<>(cmfOfficeproductbaofeiService.selectCmfOfficeproductbaofeiList(cmfOfficeproductbaofei), HttpStatus.OK);
    }
}