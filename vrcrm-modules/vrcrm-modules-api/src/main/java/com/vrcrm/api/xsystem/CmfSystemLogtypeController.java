...
@RestController
@RequestMapping("/api")
public class CmfSystemLogtypeController {
    ...
    @GetMapping(value = "/xsystem/systemlogtype/find/{id}")
    public ResponseEntity<Object> find(@PathVariable("id") Integer id) throws IOException {
        return new ResponseEntity<>(cmfSystemLogtypeService.selectCmfSystemLogtypeById(id), HttpStatus.OK);
    }
     ...
     * uniapp端调用
     *
     * @param cmfSystemLogtype
     * @return
     */
    @ApiResponse(code = 400, message = "获取帐号操作列表失败")
    @ApiOperation(value = "帐号操作列表", notes = "获取帐号操作列表")
    @GetMapping(value = "/xsystem/systemlogtype/info/")
    public ResponseEntity<Object> info(CmfSystemLogtype cmfSystemLogtype) throws IOException{
        return new ResponseEntity<>(cmfSystemLogtypeService.selectCmfSystemLogtypeList(cmfSystemLogtype), HttpStatus.OK);
    }
}