...
@RestController
@RequestMapping("/api")
public class CmfFixedassettiaokuController {
    ...
    @GetMapping(value = "/political/fixedassettiaoku/find/{id}")
    public ResponseEntity<Object> find(@PathVariable("id") Integer id) throws IOException {
        return new ResponseEntity<>(cmfFixedassettiaokuService.selectCmfFixedassettiaokuById(id), HttpStatus.OK);
    }
     ...
     * uniapp端调用
     *
     * @param cmfFixedassettiaoku
     * @return
     */
    @ApiResponse(code = 400, message = "获取帐号操作列表失败")
    @ApiOperation(value = "帐号操作列表", notes = "获取帐号操作列表")
    @GetMapping(value = "/political/fixedassettiaoku/info/")
    public ResponseEntity<Object> info(CmfFixedassettiaoku cmfFixedassettiaoku) throws IOException{
        return new ResponseEntity<>(cmfFixedassettiaokuService.selectCmfFixedassettiaokuList(cmfFixedassettiaoku), HttpStatus.OK);
    }
}