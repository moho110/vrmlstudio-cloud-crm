...
@RestController
@RequestMapping("/api")
public class CmfOfficeproducttuiController {
    ...
    @GetMapping(value = "/political/officeproducttui/find/{id}")
    public ResponseEntity<Object> find(@PathVariable("id") Integer id) throws IOException {
        return new ResponseEntity<>(cmfOfficeproducttuiService.selectCmfOfficeproducttuiById(id), HttpStatus.OK);
    }
     ...
     * uniapp端调用
     *
     * @param cmfOfficeproducttui
     * @return
     */
    @ApiResponse(code = 400, message = "获取帐号操作列表失败")
    @ApiOperation(value = "帐号操作列表", notes = "获取帐号操作列表")
    @GetMapping(value = "/political/officeproducttui/info/")
    public ResponseEntity<Object> info(CmfOfficeproducttui cmfOfficeproducttui) throws IOException{
        return new ResponseEntity<>(cmfOfficeproducttuiService.selectCmfOfficeproducttuiList(cmfOfficeproducttui), HttpStatus.OK);
    }
}