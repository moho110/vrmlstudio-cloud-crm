...
@RestController
@RequestMapping("/api")
public class CmfFixedassetweixiuController {
    ...
    @GetMapping(value = "/political/fixedassetweixiu/find/{id}")
    public ResponseEntity<Object> find(@PathVariable("id") Integer id) throws IOException {
        return new ResponseEntity<>(cmfFixedassetweixiuService.selectCmfFixedassetweixiuById(id), HttpStatus.OK);
    }
     ...
     * uniapp端调用
     *
     * @param cmfFixedassetweixiu
     * @return
     */
    @ApiResponse(code = 400, message = "获取帐号操作列表失败")
    @ApiOperation(value = "帐号操作列表", notes = "获取帐号操作列表")
    @GetMapping(value = "/political/fixedassetweixiu/info/")
    public ResponseEntity<Object> info(CmfFixedassetweixiu cmfFixedassetweixiu) throws IOException{
        return new ResponseEntity<>(cmfFixedassetweixiuService.selectCmfFixedassetweixiuList(cmfFixedassetweixiu), HttpStatus.OK);
    }
}